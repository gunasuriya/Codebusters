...
if(!empty($_POST)){
	...
	if($lang=='C'){
		$answer="";
		$goi = $_POST['inputa'];
		$file="c/".$_POST['filename'].".c";
		$cinput="c/".$_POST['filename']."_input.txt";
		$coutput="c/".$_POST['filename']."_output.txt";
		$mass=$_POST['filename'].".exe";
		file_put_contents($file, $current);
		file_put_contents($cinput, $goi);
		putenv("PATH=compile\MinGW64\bin");
		$mass=$mass." < ".$cinput;
		$kat="Error Occured\n".shell_exec("gcc c/".$_POST['filename'].".c -o ".$_POST['filename'].".exe 2>&1");
		if(strlen($kat)>15){
			$answer="";
		}
		else{
	...
	if($lang=='C++'){
		...
		$kat="";
		$goi = $_POST['inputa'];
		$file="cpp/".$_POST['filename'].".cpp";
		$cinput="cpp/".$_POST['filename']."_input.txt";
		$coutput="cpp/".$_POST['filename']."_output.txt";
		$mass=$_POST['filename'].".exe";
		file_put_contents($file, $current);
		file_put_contents($cinput, $goi);
		putenv("PATH=compile\MinGW64\bin");
		$mass=$mass." < ".$cinput;
		$kat="Error Occured\n".shell_exec("g++ cpp/".$_POST['filename'].".cpp -o ".$_POST['filename'].".exe 2>&1");
		if(strlen($kat)>15){
			$answer="";
		}
		else{
    ...
    <nav class="navbar navbar-default" role="navigation">
      <div class="navbar-header">
        <button style="padding-top:4%;" type="button" id="butt" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse">
				...
				<a  href="index.php" class="navbar-toggle" style="font-size:20px;font-family:cons;color:turquoise;font-weight:bold;border:none;float:left;top:0;text-decoration:none;">CodeBusters</a>
      </div>
      <div class="navbar-collapse collapse">
        <ul class="nav navbar-nav">
          ...
          <li class="navbar-right"><a href="index.php"  style="padding-top:15px;font-size:18px;font-family:cons;color:turquoise;">Home</a></li>
...
<h1 id="fun">LETS CODE</h1>
	...
	<form id="form1" method="POST" action="#console" autocomplete="on">
	...
	//console.log($("#lang").val());
		 ...
		 function urlda(){
				...
				console.log("localhost/project/"+lang+"/"+filename);
					...
					$.ajax({
						type: "GET",
						url: "http://localhost/project/"+lang+"/"+filename,
						success: function(data){
							if(req!="http://localhost/project/code.php"){
								console.log(data);
							}
				...
				function refineURL()
{
    ...
    return "project/"+beforeQueryString;
}
				var x = filename;
